...
"   call Decho("amiga",'~'.expand("<slnum>"))
   ...
   if ec != '/' && ec != ':'
    let ret = fnameescape(a:base) . "/" . a:subdir
   else
    let ret = fnameescape(a:base) . a:subdir
   endif
...
"   call Decho("remote linux/macos",'~'.expand("<slnum>"))
   let urlbase = substitute(fnameescape(a:base),'^\(\a\+://.\{-}/\)\(.*\)$','\1','')
   let curpath = substitute(fnameescape(a:base),'^\(\a\+://.\{-}/\)\(.*\)$','\2','')
   if a:subdir == '../'
    if curpath =~ '[^/]/[^/]\+/$'
...
"   call Decho("local linux/macos",'~'.expand("<slnum>"))
   let ret = substitute(fnameescape(a:base)."/".a:subdir,"//","/","g")
   if a:base =~ '^//'
